...
const allDeals = [
    {
        ...
        price: 159.99,
        discount: 50,
        dealEndTime: "2025-05-26T00:01:10Z",
        variants: {
...
const DealDetailPage = () => {
    const { id } = useParams();
    const deal = allDeals.find((d) => d.id === parseInt(id));
    ...
    const [mainImage, setMainImage] = useState(deal?.image[0]);
    ...
    const stockValue =
        selected.color && selected.size
            ? deal?.variants?.[selected.color]?.[selected.size] ?? 0
            : 0;

    // When color or size changes, adjust quantity if needed
    useEffect(() => {
        if (selected.color && selected.size) {
            const newStock = deal?.variants?.[selected.color]?.[selected.size] ?? 0;
    ...
    }, [selected.color, selected.size, deal]);

    // Initialize cartItem from deal
    useEffect(() => {
        if (deal) {
            setMainImage(deal.image[0]);
            setCartItem({
                ...deal,
                price: discountedPrice,
                quantity: 1,
            });
            setSelected({ color: null, size: null }); // Reset selections on deal change
            setQuantity(1);
            setShowCart(false);
            setRating(0);
            setErrorMessage(null);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            const dealEnd = new Date(deal.dealEndTime).getTime();
        ...
        <div className="container my-5">
            <div className="row">
                ...
                <div className="col-md-5 position-relative" style={{ height: "500px" }}>
                    ...
                    {deal.discount > 0 && (
                        <div
                            className="position-absolute top-0 end-0 m-2 bg-danger text-white fw-bold px-2 py-1 rounded"
                            style={{ zIndex: 2 }}
                        >
                            −{deal.discount}%
                        </div>
                    ...
                    {timeLeft && (
                        <div
                            className="position-absolute bottom-0 m-2 fw-bold px-2 py-1 rounded text-center"
                            style={{ zIndex: 2, backgroundColor: "lightgray", color: "gray" }}
                        >
                ...
                >
                    <div>
                        <h5 className="text-uppercase text-muted">ZVEZDA</h5>
                        <h2>{deal.name}</h2>
                        <h6 className="text-secondary">Category: {deal.category}</h6>
                        ...
                        <Price value={discountedPrice} oldValue={deal.discount ? deal.price : null} />
                        ...
                        <div className="my-3">
                            ...
                            <div className="d-flex gap-2 mt-2">
                                {Object.keys(deal.variants).map((clr, i) => (
                                    <div
                                        key={i}
